...
export export_identity, reassemble_identity

using Libdl

# ── Library Path ──────────────────────────────────────────────────────
# Attempts to load from the 'dist' directory or standard system paths.
const LIB_NAME = "librco"
const LIBRCO = Libdl.find_library(LIB_NAME, [joinpath(@__DIR__, "..", "..", "..", "dist")])
if LIBRCO == ""
    error("Sovereign Kernel ($LIB_NAME) not found. Please run scripts/rco-shipment.sh first.")
end
...
function check_status(status::Int32, context::String)
    ...
    msg = get(msgs, Int(status), "Unknown error $status")
    throw(RcoError(status, "$context: $msg"))
end
...
"""
    export_identity(root_hash::Vector{UInt8}, signature::Vector{UInt8}, threshold::UInt32) -> (Vector{UInt8}, Vector{UInt8})

Exports a sovereign identity into an encrypted payload and its unique identity ID.
"""
function export_identity(root_hash::Vector{UInt8}, signature::Vector{UInt8}, threshold::UInt32)
    @assert length(root_hash) == 32
    @assert length(signature) == 64
    
    payload = Vector{UInt8}(undef, 96)
    identity_id = Vector{UInt8}(undef, 32)
    
    status = ccall((:rco_identity_export, LIBRCO), Int32,
                   (Ptr{UInt8}, Ptr{UInt8}, UInt32, Ptr{UInt8}, Ptr{UInt8}),
                   root_hash, signature, threshold, payload, identity_id)
    check_status(status, "export_identity")
    return payload, identity_id
end

"""
    reassemble_identity(payload::Vector{UInt8}, identity_id::Vector{UInt8}, threshold::UInt32) -> (Vector{UInt8}, Vector{UInt8})

Reassembles a sovereign identity from an encrypted payload.
"""
function reassemble_identity(payload::Vector{UInt8}, identity_id::Vector{UInt8}, threshold::UInt32)
    @assert length(payload) == 96
    @assert length(identity_id) == 32
    
    root_hash = Vector{UInt8}(undef, 32)
    signature = Vector{UInt8}(undef, 64)
    
    status = ccall((:rco_identity_reassemble, LIBRCO), Int32,
                   (Ptr{UInt8}, Ptr{UInt8}, UInt32, Ptr{UInt8}, Ptr{UInt8}),
                   payload, identity_id, threshold, root_hash, signature)
    check_status(status, "reassemble_identity")
    return root_hash, signature
end
